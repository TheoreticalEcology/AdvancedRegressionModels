...
Note that in the code that I gave you not all predictors were scaled (and they don't say if they scaled in the paper), but as we are looking for main effects in the presence of interactions, we should definitely scale to improve the interpretability
...
OK, this means we can replicate the results of the paper, even if concentrating the entire analysis exclusive on high NDAM seems a bit cherry-picking. Another way to phrase the result is that we don't find a main effect of MasFem. However, to be fair: the current results do say that there is a significant difference at high NDAM, and such a difference, if it existed, would be important.
...
Do we need other variables that are not confounders? There are two reasons to add them:
...
## Researcher Degrees of Freedom --- Skin Color and Red Cards
...
There is no single "correct" analysis - that is the whole point of Silberzahn et al. The following is therefore not a fully worked-out solution, but **one defensible base model** to start from. Each row is a player-referee dyad, so red cards are a **count out of the number of games** the two shared; we use that as exposure via a log offset, take the **mean of the two skin-tone ratings** rescaled to 0-1 as the predictor, and add **random effects for player and referee** to respect the non-independence.

```{r}
library(EcoData); library(glmmTMB); library(DHARMa)
d = redCards
d$skin  = ((d$rater1 + d$rater2)/2 - 1) / 4     # mean rating, rescaled to 0..1
d = d[!is.na(d$skin) & d$games > 0, ]

m = glmmTMB(redCards ~ skin + (1|playerShort) + (1|refNum),
            offset = log(games), family = poisson, data = d)
summary(m)
exp(fixef(m)$cond["skin"])   # rate ratio, darkest (1) vs lightest (0) skin
res = simulateResiduals(m); plot(res); testDispersion(res)
```

The exponentiated `skin` coefficient is our effect in **rate-ratio** units (comparable to the paper's odds ratios). Compare it to the 0.89-2.93 range of odds ratios reported across the 29 teams. This bare model actually gives a somewhat *higher* estimate, which nicely illustrates the point of the study: the "researcher degrees of freedom" (choice of exposure, random-effect structure, which rater, count vs. binomial, and - crucially - which confounders are included) move the result substantially. From this base model, you would go on to check residuals and dispersion and to add plausible confounders (league, playing position, player skill, ...), which typically pulls the estimate towards the reported range.
:::
...
Surprisingly, we find large effects of the other variables. Because of these large effects, we also test for interactions with the experimental treatment
...
We now find that there is an interaction with the main predictor, and there could be effects. We can also look at this visually.

```{r, warning=FALSE}
plot(allEffects(fit2))
```

The results are difficult to interpret. I would think that there was some bias in the experiment, which led to an effect of the Maze direction, which then creates a spill-over to the other (and in particular the main) predictors.

For our education, we can also look at the residual plots. I will use fit1, because there was a misfit:

```{r}
res <- simulateResiduals(fit1, plot = T)
```

As we would expect significant interactions, we would probably see something if we plot residuals against predictors or their interactions, but I want to show you something else:
...
```{r}
res <- simulateResiduals(fit2, plot = T)
res2 <- recalculateResiduals(res, group = dat$Colony)
...
The snails data set in the EcoData package includes observations on the distribution of freshwater snails and their infection rates ( schistosomiasis (a parasite)).

The first scientific question is that their abundance depends on the water conditions. The second scientific question is that their infection rate depends on the water conditions and seasonal factors
...
2.  Model the infection rate of all three species (Bulinus_pos_tot) (k/n binomial)
...
The seedBank data set in the EcoData package includes observation on seed bank presence and size in different vegetation plots.
...
1.  Fit an lm/lmm with SBDensity as response
...
Let's fit a base model with our hypothesis that logSBDensity \~ sSeedMass (Seed Mass) + sSeedShape + sSeedN (Number of Seeds).
...
Let's add a random slope on sAltitude:
...
Fit one of the responses in the snouter dataset against the predictors rain + djungle (see ?snouter). Check for spatial autocorrelation and proceed to fitting a spatial model if needed. See the data set's help for details on the variables.
...
Fit a base model, then check its residuals for spatial autocorrelation. We use `snouter1.1` (a continuous response) here; the coordinates in this dataset are `X` and `Y`:

```{r}
library(EcoData); library(glmmTMB); library(DHARMa)
fit = glmmTMB(snouter1.1 ~ rain + djungle, data = snouter)
res = simulateResiduals(fit); plot(res)
testSpatialAutocorrelation(res, x = snouter$X, y = snouter$Y)
```

If the spatial test is significant, we first de-trend with a 2D spline (as recommended in the spatial-correlation section - always de-trend before reaching for a correlation structure):

```{r}
library(mgcv)
fit2 = gam(snouter1.1 ~ rain + djungle + te(X, Y), data = snouter)
res2 = simulateResiduals(fit2)
testSpatialAutocorrelation(res2, x = snouter$X, y = snouter$Y)
```

This changes residuals somewhat, but there is still significant spatial autocorrelation left. Thus, we have to move to a spatial model. 
...
This is an open, integrative exercise, so the following is not a fully worked-out solution but **a base model to start from** for one of the responses (beetle counts). The other responses (`windObs` -\> Gaussian / gls, `lizards` -\> binomial, `survived` -\> k/n binomial) follow exactly the same recipe.

```{r}
library(EcoData); library(glmmTMB); library(DHARMa)
d = volcanoisland

# beetles: counts -> start with Poisson and plot as RE, then diagnose
m0 = glmmTMB(beetles2 ~ windObs + lizards + (1|plot), family = poisson, data = d)
res = simulateResiduals(m0); plot(res)
testDispersion(res); testZeroInflation(res)

# if over-dispersed / zero-inflated -> negative binomial + ZI term
m1 = glmmTMB(beetles2 ~ windObs + lizards + (1|plot),
             family = nbinom2, ziformula = ~1, data = d)

# then check for residual spatial autocorrelation on the coordinates
res1 = simulateResiduals(m1)
grp  = recalculateResiduals(res1, group = d$plot)
testSpatialAutocorrelation(grp,
      x = aggregate(x ~ plot, d, mean)$x,
      y = aggregate(y ~ plot, d, mean)$y)
```

Work through the diagnostic ladder in order - functional form -\> distribution -\> dispersion -\> zero-inflation -\> correlation - adding structure only when a residual check demands it. Note the hint that `lizards` (predators) may belong in the beetle model, and that detectability may depend on `windObs`.
:::
