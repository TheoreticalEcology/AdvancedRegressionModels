...
1.  We want to keep the regression formula y \~ f(x) of the lm with all it's syntax, inlcuding splines, random effects and all that. In the GLM world, this is called the "linear predictor".
...
The standard model for count data (1,2,3) is the Poisson regression, which assumes a Poisson distribution + log link function. Here an example:
...
Alternatively, you can use the predict() function with type = "response"
...
For count data, the log link is nearly always used. Before GLMs were widely available, was common to it lm with log transformed counts, which is basically log link + normal distribution

Poisson models are often overdispersed -\> see chapter on residual checks. Common alternative to the Poisson is the negative binomial, which is essentially a Poisson with a parameter for the variance (to address overdispersion)

If there is a variable that that controls the number of counts (e.g. time, area), this variable is usually added as a offset in the following form
