...
1.  We want to keep the regression formula $y \sim f(x)$ of the lm with all its syntax, including splines, random effects and all that. In the GLM world, this is called the "linear predictor".
...
::: {.callout-note}
As you see, I noted the link function as $link^{-1}$ in this equation. The reason is that traditionally, the link function is applied to the left hand side of the equation, i.e.
...
- Beta regression for continuous 0/1 data
...
As an example, we use the data set birdfeeding from the EcoData package - the dataset consists of observations of food given by parents to nestlings as a function of the attractiveness of the nestling.
...
The output is very similar to the `lm()`, however, as the residuals are no longer assumed to scatter normally, all statistics based on R^2^ have been replaced by the deviance (deviance = 2 [logL(saturated) - logL(fitted)]). So, we have
...
**Poisson vs. log transformed count data:** For count data, even if the distribution is switched, the log link is nearly always the appropriate link function. Before GLMs were widely available, it was common to fit an lm with log-transformed counts, which is basically log link + normal distribution

**Log offset:** If there is a variable that controls the number of counts (e.g. time, area), this variable is usually added as an offset in the following form
...
Thus, let's move to the logistic regression, which assumes a 0/1 response + logit link. In principle, this distribution is called Bernoulli, but in R both 0/1 and k/n are called "binomial", as Bernoulli is the special case of binomial where n = 1.
...
**Overdispersion:** 0/1 binomial responses cannot be overdispersed, but k/n responses can be. However, 0/1 responses can show overdispersion if grouped to k/n. Note next section on residual checks, as well as comments on testing binomial GLMs in the\
...
on habitat use of Elks in Canada. Measured is the presence of Elks (0/1), and a number of other predictors that are described in the help
...
```{r, message=FALSE}
library(MASS)
fit <- glm(presence ~ dist_roads  + dem + ruggedness, data = elk, family = "binomial")
...
```{r}
summary(glm(presence ~ dist_roads, data = elk, family = "binomial"))
```
...
plotResiduals(res, form = elk$dem, quantreg = TRUE)
plotResiduals(res, form = elk$ruggedness, quantreg = TRUE)
```

The functional forms of our confounders are not perfect.

Since we are not really interested in them, a cool trick is to use a GAM which automatically adjusts the functional form of the fitted curve to flexibly take care of the confounders. Our main predictor dist_roads is still modelled as a linear effect.

```{r}
library(mgcv)
fit2 <- gam(presence ~ dist_roads + s(dem) + s(ruggedness), data = elk, family = "binomial")
summary(fit2)
```
...
### Continuous positive response - Gamma regression

The Gamma regression is the natural choice for a **strictly positive, continuous, right-skewed** response whose variance grows with the mean (e.g. concentrations, waiting times, biomass, body sizes). A common alternative is to log-transform and fit an `lm` (as we do first below), but the two are not equivalent: the log-`lm` assumes constant variance *on the log scale* and, if you back-transform predictions, incurs a retransformation bias, whereas the Gamma GLM models the mean on the original scale with a variance that scales as $\text{Var}(y) \propto \mu^2$. With the default log link, coefficients are interpreted multiplicatively - `exp(coef)` is the factor by which the mean changes per unit of the predictor, exactly as in the Poisson regression.
...
### Continuous proportions - Beta regression and other options

We already covered discrete proportions, which can be modelled with a logistic regression. For continuous proportions, this model is not suitable, because we don't know how many "trials" we have. There are a few other options to model this data, in particular the beta regression. Let's have a look.

Use a Beta regression when the response is a **continuous proportion strictly inside (0, 1)** - a fraction, a percentage cover, a probability - as opposed to $k$ successes out of $n$ trials (that is the binomial case). The Beta distribution is bounded on (0, 1) and naturally heteroskedastic (its variance is largest near 0.5 and shrinks towards the boundaries), which the logit-normal `lm` and the arcsine transform handle poorly. One caveat: the Beta density is undefined at exactly 0 or 1, so responses containing 0s or 1s need either a small squeeze towards the interior or a zero-/one-inflated (ordered Beta) model. As with the logistic regression, effects are interpreted on the logit scale.
...
First of all: everything we said about model selection and residual checks for LMs also applies for GLMs, with only very few additions, so you should check your model in principle as before. However, there are a few tweaks you have to be aware of.
...
Out of the help page: The function creates a plot with two panels. The left panel is a uniform Q-Q plot (calling <a href="https://rdrr.io/cran/DHARMa/man/plotQQunif.html" target="_blank" rel="noopener">plotQQunif</a>), and the right panel shows residuals against predicted values (calling <a href="https://rdrr.io/cran/DHARMa/man/plotResiduals.html" target="_blank" rel="noopener">plotResiduals</a>), with outliers highlighted in red.
...
The reason is that simple GLM distributions such as the Poisson or the Binomial (for k/n data) do not have a parameter for adjusting the spread of the observed data around the regression line (dispersion), but their variance is a fixed function of the mean.
...
1.  glmmTMB models only allow unconditional residuals, which means that dispersion and zero-inflation tests are less powerful
...
It looks in the effect plots as if the slope is changing as well, but note that this is because the effect plots scale the y axis according to the link - absolutely, the effect of concentration is 10% for both groups.
...
If we turn this around, this means that if we want the model to have the same effect of concentration at the response scale for both treatments, we must implement an interaction.
...
::: {.callout-note}
If effect directions change in sign, they will do so under any link function (as they are always monotone), so changes in effect direction are robust to this problem.
:::
...
### Unconditional vs. marginal predictions
...
- Add environmental variables and potential confounders (see ?snails for an overview of the variables and if you want, you can also take a look at the original paper)
...
Year (our confounder) has a very strong effect on the occurrence rate. Conductivity has a strong positive effect. Let's look at the effects:
...
Year (our confounder) has a very strong effect on the occurrence rate. Site_type, swmo_prec (Precipitation), sTemp_Water, and sTemp_Air have strong effects:
...
The data for this example contains observations of elk presence / absence in various places in Canada.

The ecological question is if roads have a causal effect on elk presence. As we have 0/1 data, we should fit a binomial GLM (logistic regression)
...
The initial results seem to suggest that elk presence is lower close to roads. However, as we want to estimate a causal effect, we have to first establish the likely causal diagram and then correct for confounders! Tasks:

1.  Check the help of the data ?elk and consider which variables you should adjust for in a multiple GLM, based on the ideas explained in the chapter on causal analysis

2.  After specifying the multiple regression, check the residuals of your model to make sure it's properly specified, including residual plots against all predictors. If you find misfits, consider the use of splines, which can be specified by replacing the glm with the mgcv::gam function

3.  Bonus: read up on checking residuals of logistic regressions in the DHARMa vignette
...
The AUC is a common measure of goodness of fit for binary classification. However, it doesn't penalize for complexity, as the AIC. Better to look at it under cross-validation
