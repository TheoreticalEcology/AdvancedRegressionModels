...
OK, I would say there is some dependency there. To quantify this numerically, you could run
...
Here, `use = "complete.obs"` removes NA values, and `method = "kendall"` sets which correlation coefficient to calculate. What we see is that we have a negative correlation of -0.4283603.

::: callout-tip
Here, I use the Kendall rank correlation coefficient. Rank correlation coefficients work better for nonlinear correlations and are less influenced by outliers than the traditional Pearson correlation coefficient, and are thus generally to be preferred.
:::

What we want to do now is fitting a linear regression model through the data with the `lm()`{.R} function of R. We can do this in R by typing
...
1.  **Systematic misfit of the mean:** the first thing we should worry about is if the model describes the mean of the data well, i.e. if our function form (we assumed a straight line) fits to the data. You can see misfit for our model in a) the effects plot, b) the res \~ fitted plot, and c) the DHARMa res \~ fitted plot. If you see such a misfit, you should adjust your regression formula (see next section).

2.  **Distributional problems:** only if your model fits the mean of the data well should you look at distributional problems. Distributional problems means that the residuals do not scatter as assumed by the regression (for an lm iid normal). The reason why we look only after solving 1) at the distribution is that a misfit can easily create distributional problems. Nearly the entire rest of this book will deal with solving various distributional problems.
...
Once we are happy with our model, we can get a summary of the statistical estimates with the summary() function. For simplicity, I stay with the base model:
...
3.  Fit Ozone \~ Temp, and look at summary, residuals and visualizations. What would you conclude?
:::
...
but the interpretation of the results often leads to confusion. We see effects for trt1 and trt2, but where is ctrl? The answer is: what R is calculating here are so-called **treatment contrasts**. In treatment contrasts, we have a **reference level** (in our case ctrl), which is the intercept of the model, and the we estimate the **difference to the reference** for all other factor levels (in our case trt1 and trt2).
...
So, in our example, the intercept estimates the mean weight in the ctr group (with a p-value tested against zero, usually not interesting), and the effects of trt1 and trt2 are the differences of weight compared to the control, with a p-value for a test against an effect size of zero. Note that we do not get a comparison of trt1 against trt2 in this case.
...
which results in fitting the mean for each factor level. Unfortunately, this syntax option doesn't generalize to multiple regressions. In general, you can change the contrasts via the contrasts() argument.

::: callout-tip
Another way to avoid the ordering problem is running an ANOVA, possibly coupled with post-hoc tests. An ANOVA allows you to test, among other things, if a categorical predictor as a whole has an effect. More on this in the section on ANOVA in the chapter on multiple regression.
:::
