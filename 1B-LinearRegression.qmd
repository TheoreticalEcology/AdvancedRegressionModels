...
Let's start with the basics. For this chapter, we will rely a lot on the airquality data, which is built-in in R. If you don't know the data set, have a look at the description via

```{r chunk_chapter3_chunk2, echo=TRUE, eval=FALSE}
?airquality
```

and at the structure of the data via

```{r chunk_chapter3_chunk3, echo=TRUE, eval=FALSE}
str(airquality)
```

To get started, let's say we want to examine the relationship between Ozone and Wind in these data. Let's visualize this first:
...
We can get a summary of the statistical estimates with the summary() function
