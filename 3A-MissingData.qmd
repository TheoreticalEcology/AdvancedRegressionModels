...
2.  If you run a model with many variables, each of them with a number of NAs, you might lose a lot of observations and thus power. It may be better to perform an imputation (see below).
3.  If your NAs are not missing at random, you might actually have a bias in your model if you simply omit

For all these reasons, you should get a good overview about your NAs, and consider what to do with them.
...
We see that NAs are in Ozone and Solar.R, and sometimes they're together, sometimes they are separate. To check if you have a complete row, we can use the function `complete.cases()`, which returns a vector with T/F if a row is complete or not. Here, I check how many complete observations we have
...
After having an overview about the NAs in your data, as a second step in the analysis (or ideally already during the design of the experiment) is to ask yourself under which process the missingness was created. We distinguish three crucial classes of processes:

1.  MCAR = missing completely at random
2.  MAR = missing at random
3.  MNAR = missing not at random

The easiest cases is arguably MCAR = missing completely at random - in this case, the occurrences of NAs in variable X are completely random, and do not correlate with either X or other variables. This occurs when the causes of NAs are completely unrelated to the variables in the data. The opposite case is MNAR = missing not at random - in this case, the occurrences of NAs in variable X depend on the (true) value of X and possibly on other observed values in the dataset. A mix is MAR = missing at random - in this case, the occurrences of NAs in variable X do not depend on X, but possibly on other observed variables. See also [@bhaskaran2014difference].

### Imputation

The main alternative to throwing out observations with NAs is filling them up. This process is known as imputation. Multiple imputation refers to the insight that imputing several times often improves the result. There are a number of packages that do imputations, for example `missRanger`
...
Note that for proper multiple imputation the imputations must actually differ from each other - otherwise the pooled standard errors do not reflect imputation uncertainty. With `missRanger`, this is controlled by `pmm.k` (predictive mean matching): with the default `pmm.k = 0` the imputations are essentially deterministic, so we set `pmm.k = 5`, which draws each imputed value from the 5 nearest observed values and thus injects genuine between-imputation variability.

```{r, warning=F, message=F}
# run 20 imputations (pmm.k > 0 for genuine between-imputation variability)
set.seed(42)
airqualityMImp <- replicate(20, missRanger(airquality, pmm.k = 5), simplify = FALSE)
# fit 20 models
