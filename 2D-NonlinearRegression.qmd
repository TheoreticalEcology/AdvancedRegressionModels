...
Because the linear regression is much better supported in R (as in other statistical packages), it is always preferable to consider if the function you want to fit can be made linear in the parameters by transforming y or x (e.g. by regressing against log(y), logit(y)).
