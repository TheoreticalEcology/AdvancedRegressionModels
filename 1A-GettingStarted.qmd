...
- RStudio is the editor that helps you send inputs to R and collect outputs.
...
install.packages("pak") # only needed if pak is not yet installed
pak::pak("TheoreticalEcology/EcoData")
```

For your convenience, the EcoData installation also forces the installation of most of the packages needed in this book, so this may take a while. If you want to load only the EcoData package without its dependencies, run `pak::pak("TheoreticalEcology/EcoData", dependencies = FALSE)`.
...
In the remainder of this section, I will summarize basic plots and data manipulations skills in R, as they would be taught in an introductory R course (for example [here](https://theoreticalecology.github.io/DataScienceInR/)). Please read through them and make sure you know these commands in advance of the course. At the end of this part, there is a small exercise about data manipulation. If you are confident that you can do it, you can also go there directly without reading the content of this section.
...
the variable x now contains some data, namely the value 1, and I can use x as a placeholder for the data it contains in further calculations.
...
Atomic types: e.g. numeric, factor, boolean ...; List can have several types, vector not! data.frame is list of vectors. matrix is 2-dim array, array can have any dim, only one type.
:::

::: {.callout-caution icon="false"}
#### Exercise

What is the data type of the iris data set, which is built into R under the name
...
For data.frames and other objects with two or more dimensions, the same is true, except that you have several indices.
...
Data.frames are the same as matrices, except that, like with lists of vectors, you can also access columns via names as in `data.frame$column`{.R}. This is because a data.frame is a list of vectors.
...
In the previous step, `as.numeric()` coerced `"dog"` to `NA` because a string cannot be converted to a number. The column therefore already contains an `NA` before `apply()` is called. The `mean()` function with its default `na.rm = FALSE` propagates any `NA` in its input, so the result is `NA` (**N**ot **A**vailable, missing value).
...
Note that `max`{.R}\` is the function to get the maximum value.

The dot is general R syntax and usually refers to "use all columns in the data set".

#### For loops

Apply and aggregate are convenience functions for a far more general concept that exists in all programming language, which is the for loop. In R, a for loop look like this:
...
and if it is executed, it will execute 10 times the main block in the curly braces, while counting the index variable i from 1:10. To demonstrate this, let's execute a shorter for loop, going from 1:3, and printing i
...
For loops are very useful when you want to execute the same task many times. This can be for plotting, but also for data manipulation. For example, if I would like to re-program the apply function with a for loop, it would look like this:
...
I assume that you have already made plots with R. Else [here](https://www.youtube.com/embed/UXeJ1bNYCos) is a super-quick 5-min introduction video. In this course, we will not be using a lot of graphics, but it will be useful for you to know the basic plot commands. Note in particular that the following two commands are identical:
...
3.  Scaling means that the variables are centered and standardized (divided by their standard deviation):
    ...
    ```{r}
    dat$sWind = as.numeric(scale(dat$Wind))
    summary(dat)
    ```

::: {.callout-note}
`scale()` returns a one-column **matrix**, not a plain vector. Wrapping it with `as.numeric()` strips the matrix structure before assigning to a data frame column. If you skip `as.numeric()`, the column silently stores a matrix-inside-a-data-frame, which can cause unexpected errors in downstream functions that assume a plain vector (e.g., some plotting helpers).
:::
...
7.  Spearman is a rank correlation factor, less sensitive to outliers and non-linearity:
