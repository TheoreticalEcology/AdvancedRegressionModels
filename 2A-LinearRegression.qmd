...
Most datasets in R have a help file. Thus, if you don't know the data set, have a look at the description via pressing F1 with the cursor on the word, or via typing
...
::: {.callout-note}
The function name lm is short for "linear model". Remember from your basic stats course: This model is **not** called linear because we necessarily fit a straight line. It's called linear because the response (in our case Ozone) is expressed as a **linear combination of the parameters** (we call the parameters "*effects*"). Being linear in the parameters is what ensures that they all affect predictions linearly, so that the estimation can be solved as a system of linear equations. Note that the *predictors* may still be transformed (e.g. $x^2$, $\log x$, or a spline basis) - as long as the effects enter linearly, the model remains a linear model.

So, an lm is any regression of the form $y = \operatorname{f}(x) + \varepsilon$ where $\varepsilon \sim \mathcal{N}(0, \sigma^2)$, with $\operatorname{f}$ linear in the parameters, e.g. ${a}_{0} + {a}_{1} \cdot x + {a}_{2} \cdot {x}^{2}$ (the effects ${a}_{0}, {a}_{1}, {a}_{2}$ enter linearly, even though $x^2$ is a nonlinear transform of $x$). Here, $\varepsilon \sim \mathcal{N}(0, \sigma^2)$ means that the residuals (deviations from $\operatorname{f}(x)$) follow a normal (Gaussian) distribution with unknown variance $\sigma^2$.
:::

::: {.callout-tip collapse="true"}
## Understanding how the lm works

To understand how the lm works, we will estimate an lm using a custom likelihood function:
...
3.  **Test the estimated parameters against 0 (standard errors can be derived from the hessian of the MLE):** Our true parameters are 0.0 for the intercept, 1.0 for the slope, and 0.2 for the sd of the observational error:
...
pt(t_statistic, df = 100-2, lower.tail = FALSE)*2
```

::: {.callout-tip}
For a model $y = \beta_0 + \beta_1 x + \varepsilon$, there are three unknowns: $\beta_0$, $\beta_1$, and $\sigma^2$. However, the degrees of freedom for the t-test of the regression coefficients are $n$ minus the number of parameters in the mean function (i.e. not counting $\sigma^2$, which is estimated separately from the residuals). For this model that is $n - 2 = 100 - 2 = 98$. More generally, for a model with $p$ predictors plus an intercept, the residual df is $n - (p + 1)$.
:::
...
-   The *regression table* (starting with "**Coefficients**") provides the estimated parameters, one row for each fitted parameter. The first column is the estimate, the second is the standard error (approximate 95% CI: $\hat{\beta} \pm 1.96 \cdot \text{SE}$; for small samples use the exact $t_{n-p}$ quantile instead of 1.96), and the fourth column is the p-value for a two-sided test with ${H}_{0}$: "Estimate is zero". The t-value is used for calculation of the p-value and can usually be ignored.
...
1.  a straight 1-1 line, as well as n.s. of the displayed tests in the qq-plot (left) -\> evidence for the correct overall residual distribution (for more details on the interpretation of this plot, see plotQQunif)
...
Possible solution, adding a quadratic predictor and choosing a power of 0.35 transformation based on the boxcox function:
...
Another option to get the fit right is Generalized Additive Models (GAM). The idea is to fit a smooth function to data, to automatically find the "right" functional form. The smoothness of the function is automatically optimized.
...
Let's fit an `lm()`{.R} now with the categorical explanatory variable group. The syntax is the same as before
...
but the interpretation of the results often leads to confusion. We see effects for trt1 and trt2, but where is ctrl? The answer is: R is calculating so-called **treatment contrasts** for categorical predictors. In treatment contrasts, we have a **reference level** (in our case ctrl), which is the intercept of the model, and then we estimate the **difference to the reference** for all other factor levels (in our case trt1 and trt2).
...
We can run an ANOVA on the fitted model object using `aov()`. Note that we use `aov()` here rather than the more common `anova(fit)`, because the post-hoc test `TukeyHSD()` used below requires an `aov` object as input:
...
In this case, we would conclude that the variable group (3 levels) significantly improves model fit, i.e. the group overall has a significant effect, even though the individual contrasts in the original model were not significant.
...
In general, the ANOVA is more sensitive, because it tests for an overall effect (thus also has a larger n), whereas the individual contrasts test with a reduced n. Moreover, as we have seen, in the summary of a `lm()`, for \>2 levels, not all possible contrasts are tested. In our example, we estimate p-values for ctrl-trt1 and ctrl-trt2, which are both n.s., but we do not test for differences of trt1-trt2 (which happens to be significant).
...
We see that the multiple regression is not terribly wrong, but very uncertain about the true values. Some authors therefore advise to remove collinear predictors. This can sometimes be useful, in particular for predictions (see section on model selection); however, note that the effect of the removed predictor will then be absorbed by the remaining predictor(s). In our case, the univariate regression would be very sure that there is an effect, note that the CI does not cover the true effect of x1 (which was 1).
...
Alternatively, you can center with the built-in R command scale
...
which produces a more interpretable value for the intercept: when we center, the intercept of the centered variable can be interpreted as the Ozone concentration at the mean temperature. For a univariate regression, this value will also typically be very similar to the grand mean `mean(airquality$Ozone)`.
...
```{r}
airquality$sTemp = as.numeric(scale(airquality$Temp))
airquality$sWind = as.numeric(scale(airquality$Wind))
airquality$sSolar.R = as.numeric(scale(airquality$Solar.R))
```

::: {.callout-note}
`scale()` returns a one-column **matrix**, not a plain vector. Always wrap with `as.numeric()` when assigning to a data frame column; otherwise the column stores a matrix-inside-a-data-frame, which can cause unexpected behaviour in some downstream functions.
:::
...
So, let's assume we want to predict Ozone with a 95% CI for wind between 0 and 10:
...
The linear model (LM) has the beautiful property that it delivers unbiased estimates, calibrated p-values and all that based on analytical expressions that remain valid for any sample size, provided residuals are normally distributed. Unfortunately, there are very few extensions of the LM that completely maintain all of these desirable properties for limited sample sizes. The underlying problem is that for more complicated models, there are usually no closed-form solutions for the expected null or sampling distributions, or those distributions depend on the observed data. For that reason, non-parametric methods to generate confidence intervals and p-values are increasingly used when going to more complicated models, which motivates this first small excursion into non-parametric methods, and in particular cross-validation and the bootstrap.
...
Voila, we have a p-value, and it is significant. Randomization null models are used in many R packages where analytical p-values are not available, e.g., in the packages vegan (ordinations, community analysis) and bipartite (networks).
:::
...
In R, you don't have to code the bootstrap by hand, you can also use the boot package. The code will not necessarily be shorter, because you still need to code the statistics, but it offers a few convenient downstream functions for CIs etc.
...
The parametric bootstrap does the same thing, but the data is not re-sampled, but generated from the fitted model. This can be done conveniently in R as long as the regression package implements a `simulate()` function that works on the fitted model.
...
In the results, we see that the parametric bootstrap is much closer to the analytical results
...
For the parametric bootstrap, we just need a function of the data to return the estimate
...
We're seeing a bit of heteroskedasticity, maybe a tiny bit of misfit, otherwise it looks OK! If you want to do something:
...
1.  Fit an lm, trying to fit the classical relationship of Species \~ log(Area)
2.  Perform residual checks, and change the model if necessary
...
Important here is to use the scaled variables, so that effect sizes are comparable. An alternative would be to use an ANOVA, but this is only discussed in the next chapter!
...
Conclusion: NPP is slightly more important.
...
Note that for the numeric variable, the sTemp effect is the Temp effect for the first factor level, not overall. Thus, it could be n.s. although Temp does have significant effects for other growth forms. This could be solved by additionally running an ANOVA (see chapter on ANOVA) to see the effect of the variables overall is significant. 


```{r}
anova(lm(loght ~ growthform2 *  sTemp , data = plantHeight))
```

Alternatively, if you want to test if a specific growth form has a significant temperature effect, you could either run a univariate regression for this growth form
...
or, better (because you use all data at once, better df) you could fit the interaction switch from treatment to mean contrasts also for the interactions by:i
