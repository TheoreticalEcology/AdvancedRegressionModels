...
1.  Perform residual checks and modify the model if you think it is necessary. Interpret the fitted model - how would you present the results in a scientific publication?
...
Look at the solomonislands dataset in Ecodata. Let's assume we want to analyze whether the number of Species on the islands depends on the Area to the islands:
...
```{r}
model = lm(Species ~ Area, data = solomonislands)
summary(model)
```
...
The residuals look bad! There is something wrong with the functional form!

***3. Transform variables***

```{r}
model2 = lm(Species ~ log(Area+1) , data = solomonislands)
summary(model2)
plot(model2)
```

The residuals look better now! There is some remaining nonlinearity and a remaining heteroskedasticity, which we could address with more advanced modelling techniques that are introduced later in the book. 
:::
