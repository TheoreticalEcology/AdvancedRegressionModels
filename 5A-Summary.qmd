...
Packages to fit these models (see Appendix: syntax cheat sheet):
...
In statistics, we rarely use a simple analysis. We often use an entire pipeline, consisting, for example, of the protocol that I sketched above. What we should constantly ask ourselves: Is our pipeline good? By "good", we typically mean: If 1000 analyses are run in that way:
...
All we have learned in this course directly informs experimental and research design, be it controlled experiments, quasi-experiments or observational studies.
...
- If you are after causal inference, try to control confounders, and if not, try to measure them. It is often possible even in observational settings to control confounders, e.g. by selecting the observations in a way that confounders are at the same value. This can be useful because, as we discussed, adjusting for collinear variables is possible, but costs power.

- In experimental settings, blocked designs where blocks will be later treated as a RE are standard

If you have decided on the selection of variables, treat your analysis as fixed. Perform a pilot study and/or a power analysis for the entire study.

The cleanest way to do a power analysis for any model - not just the textbook cases - is by simulation: assume an effect size, simulate many datasets, fit your planned model to each, and count how often the effect comes out significant. That fraction *is* the power.

```{r chunk_chapter7_chunk7, echo=TRUE, eval=TRUE}
getP = function(n = 50, effect = 0.3){
  x = runif(n)
  y = effect * x + rnorm(n)          # assumed true effect + noise
  summary(lm(y ~ x))$coefficients[2, 4]
}

mean(replicate(1000, getP(n = 50, effect = 0.3)) < 0.05)   # power at n = 50
```

To find the sample size for, say, 80% power, wrap this in a loop over `n`. The same recipe works for GLMMs, spatial models, etc. - just swap the data-generating step and the fitted model, which is why simulation beats closed-form power formulas once you leave the simple LM.

## Next steps

This book covered the core GLMM toolbox. Natural next steps are the fully Bayesian treatment of these models (`brms` / Stan), which sidesteps several of the degrees-of-freedom and p-value problems we flagged along the way; machine-learning approaches when the goal is purely prediction; and domain-specific extensions such as joint species distribution models, survival / time-to-event models, and structural equation models - each of which builds directly on the ideas developed here.
