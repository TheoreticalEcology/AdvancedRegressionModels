...
| `y ~ te(x1, x2)` | tensor spline (works only for mgcv) |
...
| Type | Formula | Interpretation |
|----|----|----|
| Spatial | `gls(y~.., correlation = corExp(form = ~ Lat + Long|group))` | Spatial correlation structure (no random effects) between sites within group (group is optional) |
| Temporal | `gls(y~.., correlation = corAR1(form = ~ years | group))` | Temporal correlation structure (no random effects) between years within group (group is optional) |
| Phylogeny | `library(ape); gls(y~.., correlation = corBrownian(phy = phyTree, form = ~ species | group))` | Phylogenetic correlation structure (no random effects) between species within group (group is optional) |
