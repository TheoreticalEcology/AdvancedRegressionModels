...
```{r}
## Spatial
res2 = recalculateResiduals(res, group = droplevels(data$site_irn),)
groupLocations = aggregate(cbind(data$sLat, data$sLon ), list(droplevels( data$site_irn)), mean)
...
model2 = glmmTMB(Bulinus_tot~
                 ...
                 (1|locality/site_irn) +  (1|coll_date) + exp(0+numFac|site_irn), 
...
better but still not perfect, there is still underdisperson:
