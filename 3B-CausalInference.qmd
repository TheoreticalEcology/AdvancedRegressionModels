...
- Solar.R could affect both Temp, Ozone -\> Confounder, include
- Wind could affect Temp, Ozone -\> Confounder, include. Alternatively, one could assume that Temp is also affecting Wind, then it's a mediator
...
in this case, plot suggest to adjust for fire severity, to isolate the direct effect of cover on richness.
...
Structural equation models (SEMs) are models that are designed to estimate entire causal diagrams. For GLM responses, you will currently have to estimate the DAG (directed acyclic graph) piece-wise, e.g. with <a href="https://cran.r-project.org/web/packages/piecewiseSEM/vignettes/piecewiseSEM.html" target="_blank" rel="noopener">https://cran.r-project.org/web/packages/piecewiseSEM/vignettes/piecewiseSEM.html</a>.

For linear (= normally distributed) SEMs, we can estimate the entire DAG in one go. This also allows us to have unobserved variables in the DAG. One of the most popular packages for this is `lavaan`. The advantage of lavaan is that you can also have unobserved covariates which can be modelled latent.
...
Another way to get those effects is the following, where we fit all variables against the confounder. This essentially conditions on the confounder. If we then regress the residuals without the confounder, we get the original effect.
...
::: {.callout-note}
Strictly speaking, what we did here is the classic Frisch-Waugh-Lovell partialling-out result, not yet full double/debiased machine learning as introduced by Chernozhukov et al. (2018). What turns this trick into "double ML" is two additional ingredients: (i) the two nuisance regressions (x \~ c and y \~ c) are fit with flexible, potentially high-dimensional ML estimators (e.g. random forests, gradient boosting) instead of a simple lm, and (ii) cross-fitting - the nuisance models are trained on one part of the data and the residuals (and thus the effect of interest) are computed on a held-out part. Cross-fitting is what removes the overfitting bias that flexible ML nuisance models would otherwise introduce into the residuals, and it is what gives double ML its statistical guarantees (valid confidence intervals even when the nuisance functions are estimated nonparametrically).
:::
...
- Infant mortality could be a mediator or a collider, depending on whether you think fertility -\> infant mortality or infant mortality -\> fertility. I would tend to see it as a mediator.
...
- Nearly all other variables are confounders, gesund_org could also be a collider
