...
-   If you want to test for significance and partition R2 in a fair way, you should either choose type II or type III ANOVA. Discussions about which is preferable are ongoing, see e.g. [@hector2010analysis]. As you will see when reading this, it also depends on the purpose of what you want to do: do you want to partition R2 between model components, test significance of factors, ...

::: callout-caution
Note that for categorical predictors, apart from collinearity and balance, also the choice of contrasts (see section on contrasts) will influence the amount of shared variation, in particular regarding the variance component shared between main effects and the interactions. When using orthogonal contrasts, this shared component is minimized. Because of this, it is usually advised to use orthogonal contrasts (note: the default treatment contrasts are not orthogonal) when running a type III ANOVA with 2-way interactions between two categorical variables.
...
Note that the Helmert contrasts match much better with the results of the model without interaction. 
